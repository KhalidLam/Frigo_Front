...
import {
  ...
  AddToListe,
  DeleteRecipe,
  RemoveProductfromFrigo,
} from "./FunctionComponents/RecetteFunction";

export default function Recipe(props) {
  const { MissProducts } = useContext(MissingContext);
  ...
  useEffect(() => {
    getRecipeWithProduct(recetteId)
      .then((response) => {
  ...
  }, [changeProduct, Ingredient, products]);

  const RemovefromFrigo = (e) => {
    RemoveProductfromFrigo(Ingredient)
      .then((res) => {
        toast({
          title: "La recette est réalisée",
          description:
            "les quantités de cette recette a été retirées de votre frigo ",
          status: "success",
          duration: 2500,
          isClosable: true,
        });
      ...
      .catch((err) => {
        ...
        toast({
          title: "Erreur",
          description:
            "les produits de cette recette n'existe pas dans votre frigo ",
          status: "error",
          duration: 2500,
          isClosable: true,
        });
  ...
  const HandelAddToListe = () => {
    AddToListe(Ingredient)
      .then((res) => {
        toast({
          // title: "",
          description:
            " les ingredients de cette recette a été bien ajouter à votre liste de courses",
          status: "success",
          duration: 2500,
          isClosable: true,
        });
                  ...
                  <div data-uk-grid>
                    <div className="uk-width-auto@s uk-text-small">
                      <p className="uk-margin-small-top uk-margin-remove-bottom">
                        Created by <a href="#user"> {User} </a>
                      </p>
                    </div>
                    {recipe.user_id === localStorage.userId && (
                      <Form
                        className="  container p-4"
                        onSubmit={(e) => SubDeleteRecipe(e)}
                        action=""
                      >
                        <ButtonSave
                          color="btn-danger"
                    ...
                    <div className="uk-grid-large" data-uk-grid>
                      ...
                      <div className="col-4 ml-4">
                        <Card.Title className="text-center title d-flex justify-content-between">
                          ...
                          {recipe.user_id === localStorage.userId && (
                            <FcPlus
                              ...
                              onClick={() =>
                                history.push("/AddProductsToRecipe")
                              }
                            />
                          )}
                        </Card.Title>
                        ...
                        <ul className="uk-list uk-list-large uk-list-divider uk-margin-medium-top">
                          <IngredientCard
                            ingredient={recipe.products && recipe}
                            change={change}
                            setchange={setChange}
                          />
