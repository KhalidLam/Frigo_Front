...
export const getFilterRecettes = async () => {
  return await Axios.get(
    `http://localhost:1000/api/recettesHome/${localStorage.frigoId}`
  );
};
...
export const getMesRecettes = async () => {
  return await Axios.get(
    `http://localhost:1000/api/mesrecettes/${localStorage.userId}`,
    {
      headers: {
        Authorization: "Bearer " + localStorage.usertoken,
        "Content-Type": "application/json",
...
export const getMesFavoris = async (iconId) => {
  return await Axios.get(
    `http://localhost:1000/api/favoris?recettes_id=[${iconId}]`,
    {
